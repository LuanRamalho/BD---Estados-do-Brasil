...
if (!localStorage.getItem('state_Brazil')) {
    localStorage.setItem('state_Brazil', JSON.stringify(states));
}

function searchState() {
    const searchInput = document.getElementById('search-state').value.toLowerCase();
    const storedStates = JSON.parse(localStorage.getItem('state_Brazil'));
    const flagContainer = document.getElementById('flag-container');
    flagContainer.innerHTML = '';

    Object.keys(storedStates).forEach(region => {
        storedStates[region].forEach(state => {
            if (state.name.toLowerCase().includes(searchInput)) {
                const img = document.createElement('img');
                img.src = state.flag;
                img.alt = `Bandeira de ${state.name}`;
